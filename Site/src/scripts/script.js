...
const body = document.querySelector("body"),
    ...
    firstSection = document.querySelector(".partie1"),
    quadrillageSection = document.querySelector(".quadrillage"),
    ...
    quadrillageImgs = document.querySelectorAll(".quadrillage .quadrillage-image-container img"),
    boxModel = document.querySelector(".quadrillage .box-model"),
    quadrillageImageContainer = document.querySelector(".quadrillage-image-container"),
    boxModelArrow = document.querySelector(".quadrillage .box-model .arrow"),
    boxModelImage = document.querySelector(".quadrillage .box-model img"),
...
linkeu.forEach(link =>
    link.addEventListener("click", () => {
        alert("Non ne faites pas ça les réseaux nous manipulent ! 😱😨");
    })
);
...
// Changement du logo onscroll
window.onscroll = function() {
    
    // home page JS
    if (pageTitle.text === "VroumVroumGo - BlaBlaCar") {
        ...
        if (window.pageYOffset < headerSection.offsetHeight * 0.5) {
            ...
            window.pageYOffset < firstSection.offsetTop * 0.68
        ) {
            dots.forEach(dot => dot.classList.add("black"));
        } else if (
            window.pageYOffset > firstSection.offsetTop * 0.68 &&
            window.pageYOffset < quadrillageSection.offsetTop * 0.76
        ) {
            dots.forEach(dot => dot.classList.remove("black"));
    ...
    function boxModelFun(e) {
        ...
        if (boxModel.classList.contains("active")) {
            if (
                ...
                const arr = Array.from(quadrillageImageContainer.children);
